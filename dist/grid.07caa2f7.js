...
Promise.all([d3.csv(require('../data/rejected-plates.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  var row = 0;
  svg.selectAll('rect').data(data).enter().append('rect').classed("bad", function (d) {
    return d.plate == "BAD2SS";
  }).attr('height', 15).attr('width', 30).attr('fill', '#fff').attr('stroke', '#9F303C').attr('stroke-width', 2).attr('opacity', 1).attr("x", function (d, i) {
    return (i % 15 + 1) * 36 - 1;
  }).attr("y", function (d, i) {
    if (i % 15 == 0) {
      row++;
  ...
  svg.selectAll('text').data(data).enter().append("text").text(function (d) {
    ...
    return d.plate;
  }).attr('class', 'all_plates_text').attr("x", function (d, i) {
    return (i % 15 + 1) * 36 + 14;
  }).attr("y", function (d, i) {
    if (i % 15 == 0) {
      row2++;
    ...
    ;
    return row2 * 20 + 10;
  }).attr("font-family", "sans-serif").attr("font-size", "5px").attr("text-anchor", "middle"); // d3.select('#one').on('stepin', () => {
  //     svg
  //       .selectAll('rect')
  //       .transition()
  //       .duration(800)
  ...
  d3.select('#two').on('stepin', function () {
    svg.selectAll('rect').transition().duration(800).attr('opacity', 0.1);
    svg.selectAll('.bad').transition().duration(200).attr('opacity', 1).attr('height', 30).attr('width', 60);
    svg.selectAll('.all_plates_text').transition().duration(800).attr('opacity', 0.1);
    svg.selectAll('#BAD2SS').transition().duration(800).attr('opacity', 1).attr('font-size', '14px').attr('transform', 'translate(' + 14 + ',' + 10 + ')');
  });
}
},{"d3":"../node_modules/d3/index.js","../data/rejected-plates.csv":"../data/rejected-plates.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","grid.js"], null)
//# sourceMappingURL=/grid.07caa2f7.map